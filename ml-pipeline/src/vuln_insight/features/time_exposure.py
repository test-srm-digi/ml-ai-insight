...
def extract_time_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Parse dates
    pub = pd.to_datetime(df.get("published_date", None), errors="coerce", utc=True)
    mod = pd.to_datetime(df.get("modified_date", None), errors="coerce", utc=True)
    det = pd.to_datetime(df.get("detection_time", None), errors="coerce", utc=True)
    ...
    refs = df.get("references", pd.Series("", index=df.index)).astype(str).str.lower()
    num_refs = pd.to_numeric(df.get("num_references", 0), errors="coerce").fillna(0)
    ...
    severity_num = df.get("severity_numeric", pd.Series(0, index=df.index))
    ...
    num_sources = pd.to_numeric(df.get("num_sources", 0), errors="coerce").fillna(0)
